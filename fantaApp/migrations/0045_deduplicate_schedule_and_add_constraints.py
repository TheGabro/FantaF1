...
def _move_related(model, fk_name, source_obj, target_obj):
    ...
    unique_fields = next(
        (list(group) for group in unique_groups if fk_name in group), None
    )
    ...
    for obj in model.objects.filter(**{fk_name: source_obj}):
        if unique_fields:
            ...
            for field_name in unique_fields:
                lookup[field_name] = (
                    target_obj if field_name == fk_name else getattr(obj, field_name)
                )
...
def _merge_event_group(
    event_model, fk_name, source_weekend, target_weekend, related_models
):
    target_events = {
        event.type: event
    ...
    for source_event in event_model.objects.filter(weekend=source_weekend).order_by(
        "-id"
    ):
...
def deduplicate_schedule(apps, schema_editor):
    ...
    PlayerQualifyingMultiChoice = apps.get_model(
        "fantaApp", "PlayerQualifyingMultiChoice"
    )
    PlayerSprintQualifyingChoice = apps.get_model(
        "fantaApp", "PlayerSprintQualifyingChoice"
    )
